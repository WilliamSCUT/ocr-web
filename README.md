...
-  **响应式设计**：支持桌面端和移动端
-  **双格式输出**：同一公式同时提供 LaTeX 与 MathML，可直接粘贴到 Word/WPS

### MathML 输出

识别成功后，结果面板会同时显示 LaTeX 与 MathML 两个文本框。LaTeX 依旧用于渲染、二次编辑；MathML 则可以直接复制粘贴到 Word、WPS、Apple Pages 等文档编辑器，系统会自动插入对应的公式对象。若后端转换失败，前端会自动用 MathJax 在浏览器内做兜底转换，因此你总能拿到一个可粘贴的 MathML 版本。
